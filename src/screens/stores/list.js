...
export default function StoreList() {
    ...
    const cols = [
        ...
        {title: 'Commission', internal: 'commission', code: (row, data) => {
            return (data * 100) + '%';
        }},
        ...
        {title: 'Actions', internal: 'id', code: (row, data) => {
                return (
                    <div className={'d-flex justify-content-end'}>
                        <div className={'pe-2'} style={{cursor: 'pointer'}} onClick={() => navigate('/stores/form', {state: row})}>
            ...
            {showDelete > 0 && <DeleteModal
                ...
                <div className={'d-flex justify-content-end align-items-center'}>
                    <Button
                        ...
                        variant={'danger'}
                        onClick={() => {
                            axiosInstance.delete('store/' + showDelete + '/').then((response) => {
                                setShowDelete(0);
                                toast('Store deleted', ToastTypes.SUCCESS);
